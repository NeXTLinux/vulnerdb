...
SOURCE_REPO_URL = https://github.com/nextlinux/vulnersdb
TEMP_DIR = ./.tmp
RESULTS_DIR = $(TEMP_DIR)/results

DB_ARCHIVE = ./vulnersdb-cache.tar.gz
GRYPE_DB = go run ./cmd/$(BIN)/main.go -c publish/.vulnersdb.yaml
GRYPE_DB_DATA_IMAGE_NAME = ghcr.io/nextlinux/$(BIN)/data
...
bootstrap-tools: $(TEMP_DIR)
	...
	curl -sSfL https://raw.githubusercontent.com/nextlinux/chronicle/main/install.sh | sh -s -- -b $(TEMP_DIR)/ $(CHRONICLE_VERSION)
...
unit-go: ## Run GO unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v nextlinux/vulnersdb/test)
