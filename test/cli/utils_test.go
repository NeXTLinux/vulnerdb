...
func runGrypeDBCommand(t testing.TB, env map[string]string, expectError bool, args ...string) (*exec.Cmd, string, string) {
	...
	if !expectError && err != nil && stdout == "" {
		t.Errorf("error running vulnersdb: %+v", err)
		...
		if err != nil {
			t.Errorf("error rerunning vulnersdb: %+v", err)
			t.Errorf("STDOUT: %s", stdout)
			t.Errorf("STDERR: %s", stderr)
		}
	}
...
func getGrypeDBBinaryLocationByOS(t testing.TB, goOS string) string {
	...
	switch goOS {
	case "darwin", "linux":
		return path.Join(repoRoot(t), fmt.Sprintf("snapshot/%s-build_%s_%s/vulnersdb", goOS, goOS, archPath))
	default:
