...
For development instructions see the [DEVELOPING.md](DEVELOPING.md)

## Adding a feature or fix

If you look at the [list of vulnersdb issues](https://github.com/nextlinux/vulnersdb/issues) there are plenty of bugs and feature requests.

## Commit guidelines

In the vulnersdb project we like commits and pull requests (PR) to be easy to understand and review. Open source thrives best when everything happening is over documented and small enough to be understood.
...
You can run the static analysis and tests:

```bash
make

# same as
make static-analysis test 
```

## Pull Request

If you made it this far and all the tests are passing, it's time to submit a Pull Request (PR) for vulnersdb. Submitting a PR is always a scary moment as what happens next can be an unknown. The vulnersdb project strives to be easy to work with, we appreciate all contributions. Nobody is going to yell at you or try to make you feel bad. We love contributions and know how scary that first PR can be.
...
And lastly, when proposed changes are modifying user-facing functionality or output, it is expected the PR will include updates to the documentation as well. If nobody knows new features exist, they can't use them!