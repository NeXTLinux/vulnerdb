...
**Note**: Currently, vulnersdb is built only for Linux and macOS.

### Recommended
```bash
curl -sSfL https://raw.githubusercontent.com/nextlinux/vulnersdb/main/install.sh | sh -s -- -b /usr/local/bin
```

... or, you can specify a release version and destination directory for the installation:

```
curl -sSfL https://raw.githubusercontent.com/nextlinux/vulnersdb/main/install.sh | sh -s -- -b <DESTINATION_DIR> <RELEASE_VERSION>
```
...
vulnersdb cache backup [--path=PATH] [--provider-name=PROVIDER]

# delete all cached vulnerability data or a specific PROVIDER
vulnersdb cache delete [--provider-name=PROVIDER]

# restore vulnerability cache from a tar.gz file (PATH)
vulnersdb cache restore [--path=PATH] [--delete-existing]

# show the current state of the all vulnerability data cache or a specific PROVIDER
vulnersdb cache status [--provider-name=PROVIDER ...]
```
...
**For every schema vulnersdb supports, we build a DB for that schema nightly. To reduce nightly DB maintenance, try to keep the schema bumps to a minimum during development.**
...
This is achieved with the [Daily Data Sync](https://github.com/nextlinux/vulnersdb/actions/workflows/daily-data-sync.yaml) and [Daily DB Publisher](https://github.com/nextlinux/vulnersdb/actions/workflows/daily-db-publisher.yaml) GitHub Actions workflows.
